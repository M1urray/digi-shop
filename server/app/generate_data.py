        ...
        for subcategory in subcategories:
            for _ in range(10):  # 10 products per subcategory
                product = Product(
                    ...
                    stock=random.randint(10, 100)  # Random stock quantity for each product
                )
        ...
        for customer in customers:
            for _ in range(3):  # 3 orders per customer
                ...
                order = Order(
                    ...
                    notes=fake.sentence(),
                    is_fulfilled=False  # Ensure orders start as unfulfilled
                )
                db.session.add(order)
                db.session.commit()  # Commit to generate an order ID
                ...
                for _ in range(4):  # 4 items per order
                    ...
                    if product.stock >= quantity:
                        ...
                        product.stock -= quantity  # Deduct stock for each order item
                        db.session.add(product)  # Update product stock
                    else:
